...
<#macro first>
    <#if (data.pagesAvailable > 0)>
        <#if (data.pageNumber < 1)>
            <#local classAttr = "class=\"page-item\"" />
        <#else>
            <#local classAttr = "class=\"page-item \"" />
        </#if>
        <#local text>
...
<#--<li ${attributes}><a href="?page=${pageNumber}<#if RequestParameters.sub?exists>&sub=${RequestParameters.sub}</#if><#if RequestParameters.slug?exists>&slug=${RequestParameters.slug}</#if>">${text?html}</a></li>-->
    <li ${attributes}><a href="javascript:pagingModal(${pageNumber});" class="page-link" data-no="${pageNumber}">${text?html}</a></li>
</#macro>
...
<#macro nextHtml pageNumber text = "" attributes = "">
    ...
    <li ${attributes}><a href="javascript:pagingModal(${pageNumber});" class="page-link" data-no="${pageNumber}"><i class="mdi mdi-skip-next"></i></a></li>
</#macro>
...
<#macro previousHtml pageNumber text = "" attributes = "">
    ...
    <li ${attributes}><a href="javascript:pagingModal(${pageNumber});" class="page-link" data-no="${pageNumber}"><i class="mdi mdi-skip-previous"></i></a></li>
</#macro>
