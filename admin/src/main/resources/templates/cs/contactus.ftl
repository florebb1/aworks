                        ...
                        <div class="col d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">제휴문의 목록</li>
                                </ol>
                            </nav>
                        </div>
...
<#--                        <div class="col-auto">-->
<#--                            <button type="button" class="btn btn-primary btn-block" data-toggle="modal" data-target="#noticeModal" data-seq="">제휴문의 목록</button>-->
<#--                        </div>-->
                    </div>
                </div>
                    ...
                    <div class="form-row mb-3">
                        ...
                        <div class="col-2">
                            <select class="custom-select" name="searchType" id="searchType">
                                ...
                                <option value="comPhone">연락처</option>
                            </select>
                        </div>
                    ...
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <th scope="col">
                                <div class="d-flex">
                                    <span>이메일</span>
                                    <button type="button" class="btn btn-link border-0 p-0 ml-2 sorting" data-sort="email" data-orderby="1">
                                        <i class="icon-sort-arrows-couple-pointing-up-and-down"></i>
                                    </button>
                                </div>
                            </th>
...
<table id="contactusListByExeclTable" style="display:none;">
    <thead>
    <tr>
        <th scope="col">No</th>
        <th scope="col">이름</th>
    ...
    <tbody id="contactusListByExeclBody"></tbody>
</table>

<#include "/layout/base_script.ftl">
<script src="/assets/js/listCommon.js"></script>
<script src="/assets/js/page/cs/contactusList.js"></script>
<!-- custom script 위치 -->
<script type="text/javascript">
    ...
    function excelDownload(){
        ...
        if(chkValueArr <= 0) {
            ...
            $.ajax({
                url: "/cs/contactusExeclDownList",
                type: 'POST',
                dataType: 'JSON',
                data: datas
            }).done(function (response) {
                console.log(response);
                var contactusList = response.list;
                var html = '';
                for(var i=0;i<contactusList.length;i++) {
                    var name = contactusList[i].name;
                    if(name == null || name == "") name = "";

                    var email = contactusList[i].email;
                    if(email == null || email == "") email = "";

                    var comName = contactusList[i].comName;
                    if(comName == null || comName == "") comName = "";

                    var comPhone = contactusList[i].comPhone;
                    if(comPhone == null || comPhone == "") comPhone = "";

                    var createDatetime = getFormatDate(contactusList[i].createDatetime);
                    ...
                    html =
                        ...
                        '</tr>';
                    $("#contactusListByExeclBody").append(html);
                }

                var filename = "contactusList.xlsx";
                /* create new workbook */
                var workbook = XLSX.utils.book_new();

                /* convert table 'table1' to worksheet named "Sheet1" */
                var ws1 = XLSX.utils.table_to_sheet(document.getElementById('contactusListByExeclTable'));
