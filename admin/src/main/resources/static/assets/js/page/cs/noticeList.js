$(function(){
    ...
    $('#noticeModal').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            url: "/cs/noticeModalView?seq="+seq,
            type: 'GET'
            //ascyc: false
...
function noticeViewDetail(seq) {
    document.location.href = '/cs/noticeView?seq='+seq;
}
...
function getList(num,refresh){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for(var i =0 ; i< response.list.length;i++){
                ...
                html += '<tr>\
                           <td scope="row">\
                            ...
                            <td onclick="noticeViewDetail('+response.list[i].seq+')" style="cursor:pointer;">'+no+'</td>\
                            <td onclick="noticeViewDetail('+response.list[i].seq+')" style="cursor:pointer;">'+response.list[i].title+'</td>\
                            <td onclick="noticeViewDetail('+response.list[i].seq+')" style="cursor:pointer;">'+response.list[i].writer+'</td>\
                            <td onclick="noticeViewDetail('+response.list[i].seq+')" style="cursor:pointer;">'+moment(response.list[i].createDatetime).format("YYYY-MM-DD")+'</td>\
                            <td onclick="noticeViewDetail('+response.list[i].seq+')" style="cursor:pointer;">'+response.list[i].viewCnt+'</td>\
                            <td>\
                                <div class="more-action-dropdown">\
                                    ...
                                    <div class="more-action-dropdown-content">\
                                        <button type="button" class="more-action-item newWindowView" data-url="/cs/noticeView?seq='+response.list[i].seq+'">\
                                            <i class="icon-window-new mr-2"></i>새로운창 띄우기\
                                        </button>\
                                        <button type="button" class="more-action-item urlCopy" data-url="/cs/noticeView?seq='+response.list[i].seq+'">\
                                        ...
                                        <button type="button" class="more-action-item noticeDelete" data-seq="'+response.list[i].seq+'">\
            ...
            if(html == ""){
                html += '<tr><td colspan="7" align=center>등록된 공지사항 목록이 존재하지 않습니다.</td></tr>';
            }
